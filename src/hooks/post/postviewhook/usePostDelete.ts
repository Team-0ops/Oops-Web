...
type UseConfirmPostDeleteArgs = {
  ...
  postId?: number;
  onSuccess?: (nextPostId?: number) => void; // next를 넘겨줌
};

const pickNextPostId = (stageMap: StageMap | null, deletedPostId?: number) => {
  ...
  //방금 삭제한 글 id는 제외
  return candidates.find((id) => id !== deletedPostId);
};
...
export const useConfirmPostDelete = ({
  ...
  const getBlockMessage = () => {
    if (activeStage === "OOPS")
      return "극복 중/극복 완료 게시글이 있어 먼저 삭제해야 해요.";
    if (activeStage === "OVERCOMING")
      return "극복 완료 게시글이 있어 먼저 삭제해야 해요.";
    return "";
  };
  ...
  const { mutateAsync, isPending } = useMutation({
    ...
    onSuccess: () => {
      ...
      // ✅ 여기서 next 계산
      const nextPostId = pickNextPostId(stageMap, postId);
